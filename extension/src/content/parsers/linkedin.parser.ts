...
class LinkedInParser {
  async parse(): Promise<JobData | null> {
    try {
      ...
      const locationElement = document.querySelector('.job-details-jobs-unified-top-card__primary-description-container');
      const salaryElement = document.querySelector('.compensation__salary-range');
      const descriptionElement = document.querySelector('#job-details .jobs-description-content__text');
      ...
      // Check if remote
      const remote = !!document.querySelector('[data-job-details="remote"]') || 
